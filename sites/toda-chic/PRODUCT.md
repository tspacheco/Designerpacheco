...
- **Página inicial de clique (21/09, noite), à maneira da nortemoda.pt:** herói → vitrine de categorias → coleções → faixa de Promoções esticada → "Comprar aqui é simples" → perguntas frequentes (4 dobras + link) → a dona → Instagram → rodapé. As peças só aparecem nas páginas de listagem (`#/c/todas`, `#/c/<categoria>`, `#/e/<coleção>`). Bijuteria/Cosméticos/Casa **saíram** da loja até haver produtos. Uma só legenda por secção: "entrega em 2 a 3 dias úteis" fica apenas em "Todas as peças"; "Pagamento por MB WAY" nas coleções; lema "Embeleza-te com a nossa variedade de roupa." em rosa por baixo de "Todas as peças". Menu das 3 barras: só coleções + FAQ.
- **Pronto a lançar (22/09):** faixa "Apresentação Pacheco Studios" removida e copy de demonstração reescrita — o site deixa de ser demo. Atenção: este `index.html` é o **desenho**; não o publicar como loja viva com botão de pagar, porque não processa pagamentos. A loja a sério é o WooCommerce (`research/woocommerce-pt-passo-a-passo.md`), e o manual da dona está em `MANUAL-DONA.md`.
