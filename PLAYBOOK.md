...
- **Atenção no `render.js`:** os previews PNG usam `emulateMedia({media:"print"})` para baterem certo com o PDF; sem isso vê-se a versão de ecrã e as regras `@media print` parecem não funcionar.
...
- **Doris & Cia Pet Shop (Guarulhos, Brasil)** — `propostas/doris-e-cia/Doris-e-Cia-Plano-de-Impulso.pdf` (6 páginas, PT-BR, `build_checklist.py`, reutiliza o CSS do Be Legend): diagnóstico da ficha Google e Instagram (**categoria errada "Loja de répteis"**, sem descrição, 1 avaliação, capa com ração, dois Instagrams, sem preços públicos, sem agendamento online, sem site), checklist em 4 fases (arrumar de graça → oferta e infraestrutura → anúncios → fidelização), 3 níveis de Meta Ads (R$ 400 / 800 / 1.500 com premissas explícitas: R$ 8/conversa, 35 % conversão final, ticket R$ 80 — tabelas 2026 da Grande SP, 40 % recorrem), **sete perguntas de escala** (teto = atendimentos que cabem × preço médio; folga = teto − faturamento de hoje) com o porquê de cada uma, e próximos passos. Regra de abordagem: **nunca começar pelo orçamento**, começar pela capacidade e mostrar a folga. **Regra do Tomás: o site é extra; a venda é clientes + infraestrutura.** Reunião marcada para 23/09/2026. **PDF para a cliente:** `Doris-e-Cia-Proposta.pdf` (6 páginas, `build_apresentacao.py`): página 2 **"Como funciona"** com o funil desenhado (impressões → cliques → conversas → clientes → recorrentes, com a perda em cada degrau, custo por degrau e o que decide cada um; premissas CPM R$ 25, CTR 1,25 %, 25 % dos cliques viram conversa — reconcilia exatamente com os R$ 8/conversa) + página 3 "O potencial" com a tabela de Meta Ads em faturamento (taxa de conversão média final 35 % conversa→atendimento pago, ticket R$ 80, 40 % recorrem; barras investimento vs faturamento em 6 meses) (com **botão "Ver mês a mês"**: no ecrã abre a tabela clientes novos / que voltam / atendimentos / faturamento, com chips para trocar entre R$ 400, 800 e 1.500; ao exportar PDF a tabela fica sempre aberta no nível R$ 800 e as barras escondem-se, via `@media print`) + os 8 pontos encontrados + **3 pacotes definidos pelo Tomás — Base R$ 3.000 + R$ 200/mês · Crescimento R$ 5.000 + R$ 400/mês · Completo R$ 8.000 + R$ 900/mês** — todos com fluxo automático no WhatsApp e site incluídos; o que muda é o alcance do fluxo (Base: avaliação + lembrete 15 dias; Crescimento: + recuperação de quem sumiu, agendamento online, ficha de cliente, anúncios geridos; Completo: + saúde/aniversário, plano mensal, catálogo, 4 vídeos/mês, painel, assistência prioritária). Verba de ads e custo Meta por conversa à parte. Distribuição do conteúdo pelos pacotes foi decisão do Claude a partir das fases do plano; ajustável. **A loja JÁ TEM WhatsApp Business** (confirmado pelo Tomás, 23/09): não se monta, liga-se o sistema ao que existe. Nos documentos diz “o sistema ligado ao WhatsApp que a loja já usa”, nunca “WhatsApp Business montado”. **Cuidado técnico a confirmar com a dona:** aplicativo gratuito ou Cloud API? Os lembretes e a recuperação automáticos precisam da API, e migrar o número tira o aplicativo do telemóvel; se ela quiser manter o aplicativo, os primeiros envios ficam semiautomáticos (lista pronta, um toque para enviar). Está escrito no plano interno. **Rótulo a não errar:** a coluna do custo por aquisição chama-se **“Custo do anúncio por cliente”**, nunca “custo por cliente” — o dono lê isso como preço do serviço e estranha (aconteceu). São R$ 23 de anúncio para trazer alguém que paga R$ 80. **Modelo de recorrência:** 40 % dos clientes novos passam a vir todo mês; com 15 % de abandono mensal o total de 6 meses cai ~15 %. **Bola de neve** (reinvestir 30 % do lucro, margem 60 %): a R$ 400 iniciais fatura ~R$ 41.400 em 6 meses contra R$ 16.800 com verba fixa, mas a agenda de uma tosadora (~192 atendimentos/mês) esgota ao 6.º mês — capacidade é a pergunta a fazer à dona. Links do Google Maps e Instagram estão bloqueados nesta sessão: os dados vieram de prints enviados pelo Tomás.
...
- [ ] **Bom Paladar — carrossel de outono** — rascunho feito com fotos reais. Falta: (1) autorizar `d8j0ntlcm91z4.cloudfront.net` e correr `python3 build.py --baixar` para a versão final com azulejo ocre (imagens já geradas no Higgsfield a 24/09); (2) a Simona confirmar os três tintos e a tábua "para dois"; (3) apagar do histórico o comprovativo bancário que veio no upload (commit 659b4c4, **repo público**) — já saiu da pasta; (4) nas capturas do site de 23/07 havia 3 cartões com imagem partida ("Signature desserts", "Caramel nest", "Flambé board, live") — verificar; (5) apagar o ficheiro vazio `sites/Bom Paladar Marketing` no ramo principal. (6) Arsenal: confirmar com a Simona que as 5 sobremesas de "Cinco maneiras de acabar um jantar" continuam a sair.
