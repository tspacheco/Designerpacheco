...
- **Vídeo de herói sem CDN** (aprendido a 22/09): o CDN do Higgsfield está bloqueado nesta sessão, mas `pip install imageio-ffmpeg` traz um ffmpeg com libx264. Receita: cena generativa em canvas escrita como `render(t)` determinístico (sem estado, partículas calculadas a partir de `t` e de uma seed) → corre ao vivo no site a 0 bytes → `video.js` captura os fotogramas com Playwright e codifica MP4 16:9 e 9:16 para redes sociais. Modelo em `sites/pinhal-novo-mercado/`.
