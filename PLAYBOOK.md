...
> Última atualização: 26/09/2026 (cartão final + QR para imprimir em `marca/qr/`; ro.pachecost.com pronto a publicar — secções 10 e 15)
...
- **ro.pachecost.com** (`sites/pacheco-studios/`): página romena para onde aponta o QR do cartão. Três separadores: **Proiecte** (um quadrado por site feito, com a fonte e as cores do próprio site; abre o site com um botão «Înapoi» fixo; quadrado tracejado «Afacerea ta?» no fim), **Automatizări** (8, cada uma com a dor na voz do dono e a máquina por dentro — fluxo desenhado, regras, quem decide o quê, conversa de exemplo, pedido de demonstração; «Ce e, de fapt, AI-ul din spate?» no topo; três «caminhos» por tipo de negócio para não confundir), **Ce mai facem** (7 serviços com exemplos). Contacto com WhatsApp, email, Instagram e pachecost.com. Sem preços. Versão portuguesa para rever em `/pt/`. **Zip pronto; falta publicar no Netlify** (passos em `sites/pacheco-studios/LEIA-ME.md`).
