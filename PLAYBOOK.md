...
- **Moita (Setúbal) — cliente institucional/ONG:** croae-moita (`sites/croae-moita/`) — Centro de Recolha Oficial de Animais Errantes, gerido pelo Gabinete Veterinário Municipal. Pesquisa em `research/croae-moita.md`; arquitetura em `sites/croae-moita/ARQUITETURA.md` (direção "As 40 boxes", secção de parcerias pedida no brief). Fotos do Instagram limpas em `media/croae-moita/` (pasta de entrada dos uploads do Tomás). **Site multi-página construído (23/09):** `index` · `adotar` · `ajudar` · `parceiros` · `centro`, cada página um HTML auto-contido gerado por `ferramentas/build.py` a partir de `src/` (base.css/js, nav, footer, `pages/*.html` com blocos META/CSS/JS). Herói "As 40 boxes" (grelha 8×5, 10×4 no telemóvel), JSON-LD `AnimalShelter`, Gabarito + Atkinson Hyperlegible. Slot de vídeo em `media/video/centro.mp4|webm` (página O centro). Tel 212 806 816 / 962 049 674, gab.vetmun@cm-moita.pt. Morada, horário, fichas dos animais, parceiros e níveis de parceria a confirmar. Exceção assumida à regra single-file: o cliente pediu multi-página; cada página continua auto-contida.
...
- [ ] **CROAE Moita** — demo multi-página feita. Falta: vídeos e fotos que o Tomás tentou carregar no GitHub (não chegaram — limite de 25 MB no upload web; pedir zip ou ficheiros < 25 MB), fichas dos animais (nome, idade, porte), morada/horário, parceiros com logótipo, validar níveis de parceria e apadrinhamento de box com o Gabinete Veterinário, saber se o site é da Câmara ou de voluntários. Quando houver fotos novas: limpar com `sites/croae-moita/ferramentas/limpa-instagram.py`, copiar para `sites/croae-moita/media/`, correr `build.py`.
