...
> Última atualização: 26/09/2026 (pachecost.com passa a ser o site novo em PT · EN · RO, com ro.pachecost.com em romeno no mesmo projeto Netlify; 100 cartões impressos, falta juntar o domínio ro.pachecost.com — secções 7, 10 e 15)
...
- **pachecost.com + ro.pachecost.com** (`sites/pacheco-studios/`): **um só site em três línguas, um só projeto Netlify com dois domínios** (desde 26/09): `pachecost.com` abre em português, `pachecost.com/en/` em inglês e `ro.pachecost.com` em romeno (é para onde aponta o QR do cartão); seletor **PT · EN · RO** no topo. Três vistas: **Projetos** (só clientes reais: os de pachecost.com + Jasmim 2 + Toda Chic, cada um com a fonte e as cores do próprio site; **primeira fila Jasmim 2 · Hanam · Toda Chic** em qualquer ecrã; as apresentações a quem não comprou ficam de fora), **Automações** (8, com a dor na voz do dono e a máquina por dentro — fluxo desenhado, regras, quem decide o quê, conversa de exemplo, pedido de demonstração), **O que mais fazemos** (7 serviços). Sem preços. Medição igual ao site anterior: GoatCounter sem cookies nos dois domínios (leituras do QR contadas à parte) e pixel da Meta com faixa de consentimento só em pachecost.com. O site anterior (o «Estúdio de IA») está guardado em `sites/pachecost-com/`. Passos em `sites/pacheco-studios/LEIA-ME.md`.
- **Cartão de visita** (`marca/cartao/`): **em romeno** (mercado: Roménia), igual para todos os negócios. Propósito: *Web design · implementare sisteme AI*; telefone +351 967 117 357 (WhatsApp); frase do verso: «Nu promitem. **Arătăm.**» **PDF final pronto** (`cartao-impressao.pdf`, gráfica) + folha A4 com 10 cartões, sangria exterior e marcas de corte (`cartao-folha-a4.pdf`, casa ou reprografia; virar pela margem comprida). Contactos em `marca/dados.json`, partilhados com a página.
- **QR para imprimir** (`marca/qr/`): o mesmo QR do cartão (`ro.pachecost.com/C`) em PDF vetorial para a gráfica, SVG, PNG 600 ppp, autocolante da marca 55 × 85 mm em romeno e folha de teste A4 (5 tamanhos + régua de 100 mm). `python3 marca/qr/gerar.py` refaz tudo e lê cada QR a partir do PDF.
...
- [ ] **Cartões para a Roménia + ro.pachecost.com** (secção 15). Feito: frase escolhida, `dominio_confirmado: true`, `cartao-impressao.pdf` final, QR em `marca/qr/`, **100 cartões impressos (26/09)**, site novo em PT · EN · RO. **URGENTE: a 26/09 o `ro.pachecost.com` ainda não existia no DNS (NXDOMAIN nos servidores do Netlify): os cartões impressos leem-se mas não abrem nada.** O Tomás publicou o zip no projeto do pachecost.com (que passou a abrir em romeno); o zip novo corrige isso. **Falta, por esta ordem:** (1) Netlify → projeto do pachecost.com → *Deploys* → arrastar `sites/pacheco-studios/pacheco-studios-netlify.zip` (pachecost.com volta a abrir em português) → *Domain management* → *Add a domain* / *Add domain alias* → `ro.pachecost.com` (DNS do Netlify: o registo cria-se sozinho) → ler o QR de um cartão e ver que abre em romeno; (2) um romeno lê cartão e página; (3) se forem precisos mais cartões: `marca/qr/qr-teste-a4.pdf` a 100 % antes, e `marca/cartao/cartao-impressao.pdf` na gráfica (poucos: `cartao-folha-a4.pdf` em casa). Não dá para criar o subdomínio a partir de uma sessão do Claude: a rede bloqueia a API do Netlify e não há token (a 26/09).
- [ ] **Jasmim 2 – A Marisqueira (Manta Rota):** o site é `jasmim2marisqueira.pt` (confirmado pelo Tomás a 26/09; o `.com` não existe). Pedir o zip do site para o quadrado da página do QR ganhar a fonte e as cores dele (está em `neutro`); confirmar se entra no catálogo (secção 7) e em pachecost.com, que ainda não o tem (nem a Toda Chic).
...
- **Onde se aplica:** página RO (`sites/pacheco-studios/`) — **aplicado a 26/09:** cada automatização mostra «Cum e montată» (fluxo desenhado com declanșator/automat/persoană/decizie), «Regulile ei», «Cine decide ce» (tu · sistemul · noi), «Ce vede clientul» e «Cere o demonstrație pe WhatsApp»; o «Încearcă acum» entra quando a demo viva existir; pachecost.com (secção «como é feito», ver `research/pachecost-fraquezas.pdf`), cartão (a frase «Nu promitem. Arătăm.» já está nesta linha), conversas de venda (ouvir a dor → desenhar o fluxo à frente do dono → demo no telemóvel dele → piloto).
