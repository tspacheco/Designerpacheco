...
> Última atualização: 26/09/2026 (página do QR só com clientes reais, Jasmim 2 · Hanam · Toda Chic na primeira fila; 100 cartões impressos, ro.pachecost.com por publicar — secções 7, 10 e 15)
...
- **ro.pachecost.com** (`sites/pacheco-studios/`): página romena para onde aponta o QR do cartão. Três separadores: **Proiecte** (só clientes reais: os de pachecost.com + Jasmim 2 + Toda Chic, cada um com a fonte e as cores do próprio site; **primeira fila Jasmim 2 · Hanam · Toda Chic** em qualquer ecrã; as apresentações a quem não comprou ficam de fora; quadrado tracejado «Afacerea ta?» no fim), **Automatizări** (8, cada uma com a dor na voz do dono e a máquina por dentro — fluxo desenhado, regras, quem decide o quê, conversa de exemplo, pedido de demonstração; «Ce e, de fapt, AI-ul din spate?» no topo; três «caminhos» por tipo de negócio para não confundir), **Ce mai facem** (7 serviços com exemplos). Contacto com WhatsApp, email, Instagram e pachecost.com. Sem preços. Versão portuguesa para rever em `/pt/`. **Zip pronto; falta publicar no Netlify** (passos em `sites/pacheco-studios/LEIA-ME.md`).
...
- [ ] **Cartões para a Roménia + ro.pachecost.com** (secção 15). Feito: frase escolhida, `dominio_confirmado: true`, `cartao-impressao.pdf` final, QR em `marca/qr/`, **100 cartões impressos (26/09)**, página do QR só com clientes reais. **URGENTE: a 26/09 às 14h o `ro.pachecost.com` ainda não existia no DNS (NXDOMAIN nos servidores do Netlify): os cartões impressos leem-se mas não abrem nada.** **Falta, por esta ordem:** (1) Netlify, na conta do pachecost.com → *Deploy manually* → arrastar `sites/pacheco-studios/pacheco-studios-netlify.zip` → *Domain management* → `ro.pachecost.com` (o pachecost.com está no DNS do Netlify: o registo cria-se sozinho; `ro.pachecost.com` ainda não resolvia a 26/09) → testar `/c`, um quadrado, o «Înapoi» e a 404; (2) um romeno lê cartão e página; (3) imprimir `marca/qr/qr-teste-a4.pdf` a 100 % e ler os QR com um iPhone e um Android; (4) gráfica: `marca/cartao/cartao-impressao.pdf` (e o autocolante, se quiser); para poucos cartões, `cartao-folha-a4.pdf` em casa.
...
- **Netlify por arrastar o zip:** redirecionamentos em `_redirects` e cabeçalhos em `_headers` (funcionam sempre, também sem build); o `netlify.toml` fica só com `publish`. O gerador da página verifica que `/c` e `/C` apontam para `destino_qr`.
